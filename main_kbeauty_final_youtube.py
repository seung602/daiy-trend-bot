...
# ============================================================
PLATFORM_WEIGHTS = {
    "tiktok": 1.5, "instagram": 1.2, "youtube": 1.0, "amazon": 0.9, "google": 0.8,
}
...
CORE_SEARCH_TAGS = [
    "skincare","skincare routine","beauty skincare","skincare trends","viral skincare","trending skincare","skincare hacks","skincare ingredient",
    "viral ingredient","beauty ingredient","serum","viral serum","best serum","face serum","skin barrier","barrier repair","sensitive skin",
    "retinol","retinal","anti aging","pdrn","polynucleotide","exosome","peptide","collagen","firming","niacinamide","vitamin c","brightening",
    "acne","blemish","pore care","hyperpigmentation","dark spot","dry skin","dehydrated skin","hydrating skincare","sunscreen","sun stick",
    "spf skincare","cica","centella","snail mucin","spicule","azelaic acid","glass skin","skin flooding","skin cycling",
]
TIKTOK_QUERY_ROTATION = CORE_SEARCH_TAGS + ["skincare europe","skincare germany","beauty trends europe","beauty germany","new skincare","skin booster","aha bha skincare","slugging skincare","skinimalism","glowy skin","viral beauty","trending beauty"]
YOUTUBE_QUERY_ROTATION = CORE_SEARCH_TAGS[:]
INSTAGRAM_ROTATION = ["skincare","skincareroutine","beautyskincare","skincaretrends","viralskincare","trendingskincare","skincarehacks","skincareingredient","viralingredient","beautyingredient","serum","viralserum","bestserum","faceserum","skinbarrier","barrierrepair","sensitiveskin","retinol","retinal","antiaging","pdrn","polynucleotide","exosome","peptide","collagen","firming","niacinamide","vitaminc","brightening","acne","blemish","porecare","hyperpigmentation","darkspot","dryskin","dehydratedskin","hydratingskincare","sunscreen","sunstick","spf","cica","centella","snailmucin","spicule","azelaicacid","glassskin","skinflooding","skinscycling"]

AMAZON_QUERY_ROTATION = [
    ["skincare","facial skincare","skincare set"],["face serum","ampoule","essence"],["moisturizer","face cream","barrier cream"],["cleanser","toner","face mask"],
    ["sunscreen","sun stick","spf 50"],["eye cream","anti aging cream","brightening cream"],["retinol serum","retinal serum","bakuchiol serum"],
    ["niacinamide serum","vitamin c serum","tranexamic serum"],["pdrn serum","pdrn cream","polynucleotide serum"],["peptide serum","collagen serum","exosome serum"],
    ["ceramide cream","ectoin cream","barrier serum"],["azelaic acid","salicylic serum","snail mucin"],["propolis serum","centella serum","spicule serum"],
    ["hyaluronic serum","panthenol cream","fermented skincare"],["acne skincare","blemish serum","pore serum"],["dark spot serum","brightening serum","hyperpigmentation"],
    ["sensitive skin","redness cream","rosacea skincare"],["dry skin","dehydrated skin","hydrating serum"],["anti aging","fine lines","firming serum"],
    ["glass skin","skin barrier","barrier repair"],["skin cycling","skin flooding","slugging"],["viral skincare","trending skincare","best skincare"],
    ["best serum","best moisturizer","best sunscreen"],["skincare gift set","trending products","viral serum"],["skincare germany","sunscreen germany","skincare trends"],
...
INGREDIENTS_VOCAB = ["pdrn","polynucleotide","retinol","retinal","bakuchiol","cica","centella","niacinamide","spicule","reedle","reedle shot","peptide","copper peptide","exosome","exosomes","azelaic","azelaic acid","salicylic","panthenol","hyaluronic","collagen","ceramide","sunscreen","sunstick","sun stick","glass skin","barrier","dark spot","dark spots","hyperpigmentation","cleanser","toner","serum","moisturizer","moisturiser","essence","ampoule","mask","vitamin c","tranexamic","tranexamic acid","kojic","urea","squalane","snail","snail mucin","propolis","fermented","fermentation","volufiline","peeling","aha","bha","pha","spf","sun care","skin barrier","barrier repair","acne","antiaging","anti-aging","hydration","hydrating","brightening","glow","slugging","skin cycling","skin flooding","ectoin","pore","redness","rosacea","dry skin","dehydrated skin","sensitive skin"]
...
def init_database():
    conn = get_db()
    conn.execute("CREATE TABLE IF NOT EXISTS raw_signals (id INTEGER PRIMARY KEY AUTOINCREMENT, collected_at TEXT NOT NULL, signal_date TEXT NOT NULL, platform TEXT NOT NULL, query TEXT, tag TEXT, region TEXT, text TEXT NOT NULL)")
    conn.execute("CREATE TABLE IF NOT EXISTS keyword_daily (id INTEGER PRIMARY KEY AUTOINCREMENT, signal_date TEXT NOT NULL, keyword TEXT NOT NULL, platform TEXT NOT NULL, region TEXT NOT NULL, mentions INTEGER NOT NULL DEFAULT 0, UNIQUE(signal_date, keyword, platform, region))")
    conn.execute("CREATE TABLE IF NOT EXISTS trend_scores (id INTEGER PRIMARY KEY AUTOINCREMENT, signal_date TEXT NOT NULL, keyword TEXT NOT NULL, volume_score REAL NOT NULL, velocity_score REAL NOT NULL, persistence_score REAL NOT NULL, cross_platform_score REAL NOT NULL, regional_score REAL NOT NULL, platform_normalized_score REAL NOT NULL DEFAULT 0, trend_score REAL NOT NULL, flow_score REAL NOT NULL DEFAULT 0, z_score REAL NOT NULL DEFAULT 0, active_days_14 INTEGER NOT NULL DEFAULT 0, validation_score REAL NOT NULL DEFAULT 0, lifecycle TEXT NOT NULL DEFAULT '', UNIQUE(signal_date, keyword))")
    conn.execute("CREATE TABLE IF NOT EXISTS google_trends (id INTEGER PRIMARY KEY AUTOINCREMENT, signal_date TEXT NOT NULL, region TEXT NOT NULL, rank INTEGER NOT NULL, term TEXT NOT NULL)")
    conn.execute("CREATE TABLE IF NOT EXISTS google_signals (id INTEGER PRIMARY KEY AUTOINCREMENT, signal_date TEXT NOT NULL, collected_at TEXT NOT NULL, region TEXT NOT NULL, seed_keyword TEXT, keyword TEXT NOT NULL, query_type TEXT NOT NULL, intent TEXT NOT NULL, interest_score REAL, rising_score REAL, comparison_group TEXT, source TEXT NOT NULL, UNIQUE(signal_date, region, seed_keyword, keyword, query_type, source))")
    conn.execute("CREATE TABLE IF NOT EXISTS google_keyword_history (id INTEGER PRIMARY KEY AUTOINCREMENT, signal_date TEXT NOT NULL, region TEXT NOT NULL, keyword TEXT NOT NULL, interest_score REAL, source TEXT NOT NULL, UNIQUE(signal_date, region, keyword, source))")
    ...
    for col, typ in [("platform_normalized_score","REAL NOT NULL DEFAULT 0"), ("flow_score","REAL NOT NULL DEFAULT 0"), ("z_score","REAL NOT NULL DEFAULT 0"), ("active_days_14","INTEGER NOT NULL DEFAULT 0"), ("validation_score","REAL NOT NULL DEFAULT 0"), ("lifecycle","TEXT NOT NULL DEFAULT ''")]:
...
def send_telegram_error(msg: str):
    ...
    try: session.post(f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage", json={"chat_id": TELEGRAM_CHAT_ID, "text": "🚨 Error\n\n"+msg}, timeout=10)
    except Exception as e: logging.error("Telegram error notification failed: %s", e)

def send_telegram_message(message: str):
    if not TELEGRAM_TOKEN or not TELEGRAM_CHAT_ID: return
    url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage"
    for chunk in [message[i:i+4000] for i in range(0, len(message), 4000)]:
        try: session.post(url, json={"chat_id": TELEGRAM_CHAT_ID, "text": chunk}, timeout=15)
        except Exception as e: logging.error("Telegram send failed: %s", e)

# ============================================================
# 6. Google & Social Fetchers
# ============================================================
def fetch_google_daily_rss(geo: str, count: int = 15) -> List[str]:
    try:
        res = session.get(f"https://trends.google.com/trending/rss?geo={geo}", headers={"User-Agent": "Mozilla/5.0"}, timeout=15)
        if res.status_code != 200: return []
        return [i.text.strip() for i in ET.fromstring(res.content).findall(".//item/title") if i.text][:count]
    except: return []

def save_google_daily_rss(signal_date, region, terms):
    if not terms: return
    conn = get_db()
    for rank, term in enumerate(terms, 1): conn.execute("INSERT INTO google_trends (signal_date,region,rank,term) VALUES (?,?,?,?)", (signal_date, region, rank, term))
    conn.commit(); conn.close()

def fetch_google_autocomplete(seed, geo):
    try:
        res = session.get("https://suggestqueries.google.com/complete/search", params={"client":"firefox","q":seed,"hl":"en","gl":geo.lower()}, timeout=10)
        if res.status_code != 200: return []
        data = res.json()
        return [s.strip() for s in data[1] if isinstance(s,str) and s.strip().lower()!=seed.lower()][:10] if isinstance(data, list) and len(data) >= 2 else []
    except: return []
...
def is_beauty_relevant(text):
    t = text.lower()
    terms = ["skin","skincare","beauty","cosmetic","serum","cream","retinol","retinal","pdrn","niacinamide","peptide","exosome","sunscreen","spf","acne","barrier","toner","ampoule","essence","cleanser","mask","moisturizer","moisturiser","hyperpigmentation","dark spot","kbeauty","cica","centella","ceramide","ectoin","spicule","snail","propolis","bakuchiol","azelaic","salicylic"]
    return any(x in t for x in terms)

def save_google_signal(signal_date, region, seed, keyword, query_type, source, interest_score=None, rising_score=None):
    conn = get_db(); now = get_market_now().isoformat(); intent = classify_intent(keyword)
    conn.execute("INSERT INTO google_signals (signal_date,collected_at,region,seed_keyword,keyword,query_type,intent,interest_score,rising_score,comparison_group,source) VALUES (?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(signal_date,region,seed_keyword,keyword,query_type,source) DO UPDATE SET interest_score=excluded.interest_score, rising_score=excluded.rising_score", (signal_date,now,region,seed,keyword,query_type,intent,interest_score,rising_score,seed,source))
    conn.execute("INSERT INTO google_candidates (first_seen,last_seen,keyword,source,times_seen) VALUES (?,?,?,?,1) ON CONFLICT(keyword) DO UPDATE SET last_seen=excluded.last_seen, times_seen=google_candidates.times_seen+1", (signal_date,signal_date,keyword,source))
    if interest_score is not None: conn.execute("INSERT INTO google_keyword_history (signal_date,region,keyword,interest_score,source) VALUES (?,?,?,?,?) ON CONFLICT(signal_date,region,keyword,source) DO UPDATE SET interest_score=excluded.interest_score", (signal_date,region,keyword,interest_score,source))
    conn.commit(); conn.close()

def collect_google_independent_signals(signal_date, regions):
    output = {r: [] for r in regions}
    all_seeds = []
    for g in GOOGLE_SEED_GROUPS.values(): all_seeds.extend([_clean_text(s) for s in g])
    ...
    for seed, region in jobs:
        if is_beauty_relevant(seed): save_google_signal(signal_date, region, seed, seed, "seed", "google_seed")
        for sug in fetch_google_autocomplete(seed, region):
            ...
            output[region].append({"keyword":sug,"seed":seed,"intent":classify_intent(sug),"source":"google_autocomplete","interest_score":None,"rising_score":None})
        time.sleep(1.2)
    return output
...
def build_dynamic_keyword_pool(limit=DYNAMIC_POOL_MAX_SIZE):
    conn = get_db(); lb = (get_market_now().date() - datetime.timedelta(days=DYNAMIC_CANDIDATE_LOOKBACK_DAYS)).isoformat()
    flow_rows = conn.execute("SELECT keyword, COUNT(DISTINCT signal_date) AS days, COUNT(DISTINCT platform) AS plats FROM keyword_daily WHERE signal_date >= ? AND mentions > 0 GROUP BY keyword HAVING days >= 3 AND plats >= 2 ORDER BY days DESC, plats DESC LIMIT ?", (lb, limit*2)).fetchall()
    trend_rows = conn.execute("SELECT keyword, MAX(velocity_score) AS pv FROM trend_scores WHERE signal_date >= ? GROUP BY keyword HAVING pv >= 0.10 ORDER BY pv DESC LIMIT ?", (lb, limit*2)).fetchall()
    cand_rows = conn.execute("SELECT keyword, times_seen FROM google_candidates WHERE times_seen >= ? ORDER BY times_seen DESC, last_seen DESC LIMIT ?", (DYNAMIC_CANDIDATE_MIN_TIMES_SEEN, limit*2)).fetchall()
    conn.close()
    pool, seen = [], set()
...
def get_weekly_dynamic_pool(limit=DYNAMIC_POOL_MAX_SIZE):
    wid = get_iso_week_id(); conn = get_db()
    row = conn.execute("SELECT keywords_json FROM weekly_dynamic_pool WHERE week_id=?", (wid,)).fetchone()
    if row: conn.close(); return json.loads(row["keywords_json"]) if row["keywords_json"] else []
    pool = build_dynamic_keyword_pool(limit)
    conn.execute("INSERT INTO weekly_dynamic_pool (week_id,keywords_json,created_at) VALUES (?,?,?) ON CONFLICT(week_id) DO NOTHING", (wid, json.dumps(pool), get_market_now().isoformat()))
    conn.commit(); conn.close()
    return pool
...
def interleave_hybrid_expand(fixed, dynamic, ratio=DYNAMIC_POOL_RATIO):
    ...
    sel = df[:cnt]; sp = max(1, round(len(fixed)/len(sel)))
    res, di = [], 0
    for i, kw in enumerate(fixed):
        res.append(kw)
        if di < len(sel) and (i+1)%sp == 0: res.append(sel[di]); di += 1
    res.extend(sel[di:]); return res
...
def replace_hybrid_fixed_size(fixed, dynamic, ratio=DYNAMIC_POOL_RATIO):
    ...
    res = list(fixed); sp = max(1, len(res)//rc)
    for i in range(rc): res[min((i+1)*sp-1, len(res)-1)] = df[i]
    return res

def get_today_tiktok_queries():
...
def fetch_amazon_products():
    ...
    for qi, q in enumerate(get_today_amazon_queries()):
        ...
        except Exception as e: logging.error("Amazon '%s' failed: %s", q, e)
        if qi < AMAZON_DAILY_LIMIT-1: time.sleep(0.5)
    logging.info("Amazon samples=%d", len(results))
    return results

def _youtube_query_window():
...
def fetch_youtube_trends():
    ...
    for region in ["NL","DE"]:
        for q in queries:
            ...
            try:
                res = session.get(surl, headers=hdr, params={"key":YOUTUBE_API_KEY,"part":"snippet","q":q,"type":"video","maxResults":str(YOUTUBE_SEARCH_RESULTS_PER_CALL),"order":"date","publishedAfter":pa,"regionCode":region,"safeSearch":"none"}, timeout=15)
                sc += 1
                ...
                for item in res.json().get("items",[]):
                    ...
                    results.append({"platform":"youtube","query":q,"tag":"","region":region,"text":title.replace("\n"," ")[:220],"video_id":vid,"published_at":sn.get("publishedAt")})
            except Exception as e: logging.error("YouTube '%s' [%s] failed: %s", q, region, e)
            time.sleep(0.15)
        if sc >= YOUTUBE_SEARCH_CALLS_PER_DAY or qe: break
    ...
    for st in range(0, len(stats_ids), YOUTUBE_VIDEO_STATS_BATCH_SIZE):
        ...
        try:
            res = session.get(vurl, headers=hdr, params={"key":YOUTUBE_API_KEY,"part":"statistics,snippet","id":",".join(batch),"maxResults":str(YOUTUBE_VIDEO_STATS_BATCH_SIZE)}, timeout=15)
            vc += 1
            if res.status_code != 200:
                if res.status_code==403 and "quotaExceeded" in res.text: qe=True
                continue
...
def get_usage_month(): return get_market_now().strftime("%Y-%m")
def get_api_calls(provider, endpoint, month=None):
    conn = get_db()
    r = conn.execute("SELECT calls FROM api_usage WHERE usage_month=? AND provider=? AND endpoint=?", (month or get_usage_month(), provider, endpoint)).fetchone()
    conn.close()
    return int(r["calls"]) if r else 0
def get_apify_monthly_results():
    conn = get_db()
    r = conn.execute("SELECT COALESCE(SUM(calls),0) AS t FROM api_usage WHERE usage_month=? AND provider='apify_instagram'", (get_usage_month(),)).fetchone()
    conn.close()
    return int(r["t"] or 0)
def get_apify_daily_results(): return get_api_calls("apify_instagram", f"results:{get_today_iso()}", get_usage_month())
def add_apify_result_usage(count):
    if count <= 0: return
    conn = get_db()
    try:
        conn.execute("BEGIN IMMEDIATE")
        conn.execute("INSERT INTO api_usage(usage_month,provider,endpoint,calls,last_called_at) VALUES (?,'apify_instagram',?,?,?) ON CONFLICT(usage_month,provider,endpoint) DO UPDATE SET calls=calls+excluded.calls, last_called_at=excluded.last_called_at", (get_usage_month(), f"results:{get_today_iso()}", count, get_market_now().isoformat()))
        conn.commit()
    except: conn.rollback(); raise
    finally: conn.close()
def get_today_apify_instagram_tags():
...
def fetch_instagram_apify():
    ...
    payload = {"directUrls":[f"https://www.instagram.com/explore/tags/{t}/" for t in tags], "resultsType":"posts","resultsLimit":rl,"onlyPostsNewerThan":"3 days","addParentData":True}
    ...
    try:
        ...
        for item in data:
            ...
            results.append({"platform":"instagram","query":"","tag":src_tag,"region":"EU","text":text.replace("\n"," ")[:260],"likes":likes,"comments":comments,"views":views,"instagram_id":sc})
    ...
    except Exception as e: logging.error("Instagram failed: %s", e); return []

def save_raw_signals(signals):
    if not signals: return
    conn = get_db(); now, sd = get_market_now().isoformat(), get_today_iso()
    for s in signals: conn.execute("INSERT INTO raw_signals (collected_at,signal_date,platform,query,tag,region,text) VALUES (?,?,?,?,?,?,?)", (now,sd,s.get("platform",""),s.get("query",""),s.get("tag",""),s.get("region",""),s.get("text","")))
    conn.commit(); conn.close()

# ============================================================
# 7. Gemini JSON 호출 (Rate Limiter 적용)
# ============================================================
def _extract_json_array(text):
    text = (text or "").strip()
    if text.startswith("```"): text = re.sub(r"^```(?:json)?|```$", "", text, flags=re.DOTALL).strip()
...
# ============================================================
# 8. AI Auto Filter Engine
# ============================================================
def _get_ai_last_raw_id():
    conn = get_db()
...
def _set_ai_last_raw_id(lid):
    conn = get_db()
    conn.execute("INSERT INTO ai_filter_state (key,last_raw_id,updated_at) VALUES ('raw_signals',?,?) ON CONFLICT(key) DO UPDATE SET last_raw_id=excluded.last_raw_id, updated_at=excluded.updated_at", (lid, get_market_now().isoformat()))
    conn.commit(); conn.close()

def ai_local_prefilter(row):
...
def build_ai_filter_prompt(batch_items):
    payload = [{"id":r["id"],"platform":r["platform"] or "","query":r["query"] or "","tag":r["tag"] or "","region":r["region"] or "","text":(r["text"] or "")[:320]} for r, _ in batch_items]
    return f"""You are a cosmetics & skincare data-cleaning agent. Filter raw signals and extract beauty trend entities.
INPUT SIGNALS: {json.dumps(payload, ensure_ascii=False)}
Return ONLY a valid JSON array. Each item: {{"id":<same id>,"keep":true/false,"quality":"high"|"medium"|"low","theme":one of {json.dumps(AI_FILTER_THEMES)},"intent":one of {json.dumps(AI_FILTER_INTENTS)},"keywords":["kw1","kw2"],"confidence":0.0-1.0,"reason":"short"}}
RULES:
...
def save_ai_filter_results(signal_date, batch_items, parsed):
    ...
    conn = get_db(); now = get_market_now().isoformat(); saved = 0
    for row, thash in batch_items:
        ...
        cur = conn.execute("INSERT OR IGNORE INTO ai_filtered_signals (raw_id,signal_date,platform,region,query,tag,text_hash,text,keep,quality,theme,intent,ai_reason,confidence,filtered_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", (row["id"],signal_date,row["platform"],row["region"] or "EU",row["query"] or "",row["tag"] or "",thash,row["text"] or "",1 if keep else 0,quality,theme,intent,reason,conf,now))
        if cur.rowcount == 0: continue
        if keep and quality in AI_FILTER_KEEP_QUALITIES and kws:
            for kw in kws:
                conn.execute("INSERT INTO ai_entity_daily (signal_date,keyword,platform,region,theme,quality,intent,mentions) VALUES (?,?,?,?,?,?,?,1) ON CONFLICT(signal_date,keyword,platform,region,theme,quality,intent) DO UPDATE SET mentions=mentions+1", (signal_date,kw,row["platform"],row["region"] or "EU",theme,quality,intent))
                saved += 1
    conn.commit(); conn.close()
    return saved

def run_ai_filter(signal_date):
    if not AI_FILTER_ENABLED or not GEMINI_API_KEY: return 0
    last_id = _get_ai_last_raw_id()
    conn = get_db()
    rows = conn.execute("SELECT id,signal_date,platform,query,tag,region,text FROM raw_signals WHERE id > ? ORDER BY id ASC LIMIT ?", (last_id, AI_FILTER_MAX_SAMPLES)).fetchall()
    conn.close()
    ...
    for chunk in _chunks(batch_items, AI_FILTER_BATCH_SIZE):
        ...
        except Exception as e: logging.error("AI filter batch failed: %s", e)
...
def count_keywords_in_text(text):
    text = text.lower(); counts = Counter()
    for kw in INGREDIENTS_VOCAB:
        kw = kw.strip().lower()
        if not kw: continue
        if re.search(r"(?<!\w)"+re.escape(kw)+r"(?!\w)", text): counts[normalize_keyword(kw)] = 1
    return counts

def build_daily_keyword_counts(signals):
    counts = Counter()
    for s in signals:
        for kw in count_keywords_in_text(s["text"]): counts[(kw, s["platform"], s["region"])] += 1
    return counts

def build_daily_keyword_counts_from_ai(signal_date):
    conn = get_db()
    rows = conn.execute("SELECT keyword,platform,region,SUM(mentions) AS m FROM ai_entity_daily WHERE signal_date=? AND quality IN ('high','medium') GROUP BY keyword,platform,region", (signal_date,)).fetchall()
    conn.close()
    counts = Counter()
    for r in rows:
...
def clear_keyword_counts(signal_date):
    conn = get_db()
    conn.execute("DELETE FROM keyword_daily WHERE signal_date=?", (signal_date,))
    conn.commit(); conn.close()

def save_keyword_counts(signal_date, counts):
    if not counts: return
    conn = get_db()
    for (kw, plat, reg), m in counts.items(): conn.execute("INSERT INTO keyword_daily (signal_date,keyword,platform,region,mentions) VALUES (?,?,?,?,?) ON CONFLICT(signal_date,keyword,platform,region) DO UPDATE SET mentions=excluded.mentions", (signal_date, kw, plat, reg, m))
    conn.commit(); conn.close()

def _load_series_map(signal_date, days=28):
    conn = get_db()
    rows = conn.execute("SELECT keyword,signal_date,SUM(mentions) AS m FROM keyword_daily WHERE signal_date <= ? AND signal_date >= date(?,?) GROUP BY keyword,signal_date", (signal_date, signal_date, f"-{days} day")).fetchall()
    conn.close()
    sm = {}
...
def _flow_metrics(series):
    ...
    ema7 = _ema(lv[-7:], 0.35); ema28 = _ema(lv, 0.15)
    hist = lv[:-1]
    if hist:
...
def get_keyword_daily_history(kw, end_date, days):
    conn = get_db()
    rows = conn.execute("SELECT signal_date,SUM(mentions) AS tm FROM keyword_daily WHERE keyword=? AND signal_date<? AND signal_date>=date(?,?) GROUP BY signal_date ORDER BY signal_date", (kw,end_date,end_date,f"-{days} day")).fetchall()
    conn.close()
...
def calculate_persistence(history, window=7): return min(sum(1 for _,m in history if m>0)/window, 1.0) if history else 0.0
...
def calculate_platform_normalized_score(kw, sd):
    conn = get_db()
    rows = conn.execute("SELECT platform, SUM(CASE WHEN keyword=? THEN mentions ELSE 0 END) AS km, SUM(mentions) AS pm FROM keyword_daily WHERE signal_date=? GROUP BY platform",(kw,sd)).fetchall()
    conn.close()
...
def calculate_trend_scores(signal_date, daily_counts):
    ...
    pm_today = {}
    for (kw,plat,_),m in daily_counts.items(): pm_today.setdefault(kw,{})[plat] = pm_today.get(kw,{}).get(plat,0)+m
    results = []
    for kw in keywords:
...
def save_trend_scores(signal_date, scores):
    conn = get_db()
    for i in scores:
        conn.execute("INSERT INTO trend_scores (signal_date,keyword,volume_score,velocity_score,persistence_score,cross_platform_score,regional_score,platform_normalized_score,trend_score,flow_score,z_score,active_days_14,validation_score,lifecycle) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(signal_date,keyword) DO UPDATE SET volume_score=excluded.volume_score,velocity_score=excluded.velocity_score,persistence_score=excluded.persistence_score,cross_platform_score=excluded.cross_platform_score,regional_score=excluded.regional_score,platform_normalized_score=excluded.platform_normalized_score,trend_score=excluded.trend_score,flow_score=excluded.flow_score,z_score=excluded.z_score,active_days_14=excluded.active_days_14,validation_score=excluded.validation_score,lifecycle=excluded.lifecycle",
            (signal_date,i["keyword"],i["volume_score"],i["velocity_score"],i["persistence_score"],i["cross_platform_score"],i["regional_score"],i.get("platform_normalized_score",0),i["trend_score"],i.get("flow_score",0),i.get("z_score",0),i.get("active_days_14",0),i.get("validation_score",0),i.get("lifecycle","")))
    conn.commit(); conn.close()

# ============================================================
# 10. Reports & Gemini Generation
# ============================================================
def build_theme_rollup(date_list):
    if not date_list: return "No theme data."
    conn = get_db()
    ph = ",".join("?" for _ in date_list)
    rows = conn.execute(f"SELECT theme,keyword,SUM(mentions) AS m,COUNT(DISTINCT signal_date) AS d FROM ai_entity_daily WHERE signal_date IN ({ph}) AND quality IN ('high','medium') AND theme!='other' GROUP BY theme,keyword", date_list).fetchall()
    conn.close()
    if rows:
        themes = {}
        for r in rows:
...
def get_google_summary(signal_date, regions):
    ...
    for region in regions:
        rows = conn.execute("SELECT keyword,intent,interest_score,rising_score,source FROM google_signals WHERE signal_date=? AND region=? AND (query_type='related_candidate' OR query_type='interest') ORDER BY CASE WHEN rising_score IS NULL THEN -999999 ELSE rising_score END DESC, CASE WHEN interest_score IS NULL THEN -999999 ELSE interest_score END DESC LIMIT 20", (signal_date, region)).fetchall()
...
def get_google_candidate_list(signal_date, limit=30):
    conn = get_db()
    rows = conn.execute("SELECT keyword FROM google_signals WHERE signal_date=? ORDER BY CASE WHEN rising_score IS NULL THEN 0 ELSE rising_score END DESC, CASE WHEN interest_score IS NULL THEN 0 ELSE interest_score END DESC LIMIT ?", (signal_date, limit)).fetchall()
    conn.close()
    out, seen = [], set()
    for r in rows:
...
def build_trend_summary(scores, signal_date=None):
    ...
    for rank, item in enumerate(scores[:10], 1):
        ...
        lines.append(f"{rank}. {item['keyword']} | lifecycle={lc} | mentions={item['today_mentions']} | platforms=[{plats}] | velocity={vel_txt} | active14={item.get('active_days_14',0)}d | validation={item.get('validation_score',0):.2f} | z={item.get('z_score',0):+.1f} | cross={item['cross_platform_score']:.0f}/100 | TREND_SCORE={item['trend_score']:.1f}/100")
    return "\n".join(lines)
...
def generate_gemini_report(google_summary, google_candidates, social_data, freq_summary, trend_summary):
    ...
    if len(social_data)==0 and not google_summary.strip(): data_status = "CRITICAL DATA STATUS: No live data today. Do not fabricate rankings."
    else: data_status = f"DATA STATUS: Valid social samples: {len(social_data)}. Google discovery is independent. Do not turn autocomplete into volume claims."
    prompt = f"""You are a market-intelligence analyst tracking cosmetics/skincare consumer interest in Western Europe. NOT a K-Beauty angle. Track ingredients, formats, skin concerns regardless of origin.
Generate a DAILY WESTERN-EUROPE COSMETICS & SKINCARE TREND REPORT.
{data_status}
IMPORTANT DATA MODEL: TikTok=early viral. Instagram=secondary confirmation. Amazon=purchase-stage. Google=search discovery. YouTube=review/sustained interest. Single-platform weak signals are not confirmed trends. Google Autocomplete candidates are NOT volume scores.
{LIFECYCLE_GLOSSARY}
...
def is_weekly_summary_day(): return True if FORCE_ROLLUPS else get_market_now().date().weekday() == WEEKLY_SUMMARY_WEEKDAY
...
def get_past_month_dates(sd):
    t = datetime.date.fromisoformat(sd); f = t.replace(day=1)
...
def get_preceding_dates(date_list, count):
    ...
    conn = get_db()
    rows = conn.execute("SELECT DISTINCT signal_date FROM trend_scores WHERE signal_date < ? ORDER BY signal_date DESC LIMIT ?", (min(date_list), count)).fetchall()
    conn.close()
...
def _build_period_rollup(date_list, limit):
    ...
    conn = get_db()
    ph = ",".join("?" for _ in date_list)
    rows = conn.execute(f"SELECT keyword,SUM(trend_score) AS ts,AVG(trend_score) AS avg,MAX(trend_score) AS peak,COUNT(DISTINCT signal_date) AS ad,AVG(cross_platform_score) AS ac FROM trend_scores WHERE signal_date IN ({ph}) GROUP BY keyword", date_list).fetchall()
    conn.close()
    pd = len(date_list)
    scored = []
    for r in rows:
    ...
    return "\n".join(f"- {r['keyword']}: flow={f:.1f} | 평균={r['avg']:.1f} | 최고={r['peak']:.1f} | 지속일={r['ad']}/{pd}일" for r,f in scored[:limit]) or "No keyword data."
...
{LIFECYCLE_GLOSSARY}
WEEKLY KEYWORD RANKING (by flow score = consistency-weighted): {keyword_rollup}
...
{LIFECYCLE_GLOSSARY}
MONTHLY KEYWORD RANKING (by flow score): {keyword_rollup}
THEME ROLLUP: {theme_rollup}
...
def main():
    ...
    try:
        ...
        logging.info("=== V5 Completed Successfully ===")
