...
as ground truth for "new / rising / cooling / Korea→West transfer" —
do not invent items that aren't in it.
...
- 🌉 전이 초기 : 한국 신호 + 서구 SNS(TikTok/Instagram/YouTube)에 약하게 등장
- ✅ 서구 확인 : 여러 플랫폼(TikTok/Instagram/Amazon/Google 등)에서 동시에 확인됨
- 📉 냉각/둔화 : 한때 강했지만 이번 달 점수/언급이 크게 줄어듦
...
1. 🔄 أهم التغييرات هذا الشهر (جديد / صاعد / يبرد / أول انتقال من كوريا للغرب،
   مع وسم مرحلة كل إشارة 🇰🇷/🌉/✅/📉)
2. 🧩 التجميع حسب الثيمات (2-4 ثيمات)
3. 🏆 أفضل 5 اتجاهات مستمرة هذا الشهر (تقييم بالنجوم ★★★/★★/★)
4. 📊 تحليل مساهمة المنصات والتدفق
5. 📉 كلمات مفتاحية كانت ضجة مؤقتة فقط
6. 📌 نقاط المتابعة للشهر القادم
...
4. 📊 PLATFORM CONTRIBUTION & FLOW ANALYSIS
5. 📉 KEYWORDS THAT LOOK LIKE BRIEF NOISE/HYPE
6. 📌 NEXT-MONTH TRACKING POINTS (trend-monitoring only, 2-4 sentences)
"""
        ...
        if is_weekly_summary_day():
            try:
                ...
                weekly_report = generate_weekly_summary_report(
                    week_dates, keyword_rollup, platform_rollup, weekly_delta
                )
        ...
        if is_monthly_summary_day():
            try:
                ...
                monthly_report = generate_monthly_summary_report(
                    month_dates, keyword_rollup, platform_rollup, monthly_delta
                )
