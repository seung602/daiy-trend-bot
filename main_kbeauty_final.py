...
# Apify Instagram Scraper 설정 ($5 무료 크레딧 범위 내 차감 최소화)
INSTAGRAM_APIFY_ENABLED = True
INSTAGRAM_APIFY_LIMIT = 10  # 1회 실행 당 10개만 수집

# Amazon은 "구매 전환 신호"로 활용한다 (SNS의 화제성 신호와 상호보완).
AMAZON_DAILY_LIMIT = 3
AMAZON_QUERY_COUNT = 50  # /search 응답에서 최대로 취할 상품 수

# K-Signal (Korea Beauty & Fashion Velocity Feed) - 한국 국내 이커머스 랭킹 가속도
K_SIGNAL_MONTHLY_LIMIT = 50
K_SIGNAL_DAILY_LIMIT = 1
K_SIGNAL_FETCH_LIMIT = 50
K_SIGNAL_MIN_CONFIDENCE = 0.5
K_SIGNAL_TEASER_ENABLED = True
...
# ============================================================
# 8. TikTok (tiktok-scraper7 / tikwm) - 하루 9 calls
# ============================================================
...
# ============================================================
# 9a. Amazon - Real-Time Amazon Data (하루 3 calls)
# ============================================================
...
# ============================================================
# 9c. K-Signal (Korea Beauty & Fashion Velocity Feed)
# ============================================================
...
def fetch_k_signal() -> List[Dict]:
    ...
    try:
        res=session.get(url,headers=headers,params={"limit":str(K_SIGNAL_FETCH_LIMIT),"min_confidence":str(K_SIGNAL_MIN_CONFIDENCE)},timeout=15)
        if res.status_code!=200:
...
# ============================================================
# 10. Apify Instagram Scraper (apify/instagram-scraper)
# ============================================================

def fetch_instagram_apify() -> List[Dict]:
    if not INSTAGRAM_APIFY_ENABLED or not APIFY_TOKEN:
        logging.warning("APIFY_TOKEN missing or Instagram disabled. Instagram skipped.")
        return []

    tag = get_today_instagram_tag()
    client = ApifyClient(APIFY_TOKEN)

    run_input = {
        "directUrls": [f"https://www.instagram.com/explore/tags/{tag}/"],
        "resultsType": "posts",
        "resultsLimit": INSTAGRAM_APIFY_LIMIT,
    }

    try:
        logging.info("Executing Apify Instagram Scraper for hashtag #%s...", tag)
        ...
        for item in dataset_items:
            ...
            likes = item.get("likesCount") if item.get("likesCount") is not None else item.get("likeCount")
            comments = item.get("commentsCount") if item.get("commentsCount") is not None else item.get("commentCount")
            views = item.get("videoViewCount") or item.get("videoPlayCount") or item.get("playsCount")
        ...
        logging.info("Apify Instagram #%s samples=%d", tag, len(results))
        return results

    except Exception as e:
        logging.error("Apify Instagram #%s failed: %s", tag, e)
        return []
...
2. Instagram Scraper (Apify) = secondary social/content confirmation.
...
Amazon maximum is 3 API calls, Instagram (Apify) maximum is 10 posts, and
K-Signal maximum is 1 API call (a single batch of up to 50 Korea-domestic
ranking signals), so social sampling, while broader than before, is still
...
WEEKLY_SUMMARY_WEEKDAY = 5  # Mon=0 ... Sat=5, Sun=6
...
def main():
    ...
    try:
        ...
        k_signal_teaser = fetch_k_signal_teaser(k_signal_signals)
...
# Instagram Statistics API - 월 50 requests.
# 매일 1회 baseline을 사용하고, 월말/이상징후 재검증용 reserve를 남긴다.
INSTAGRAM_STATS_ENABLED = True
INSTAGRAM_STATS_MONTHLY_LIMIT = 50
INSTAGRAM_STATS_QUERY_COUNT = 30
...
def fetch_instagram_statistics() -> List[Dict]:
    if not INSTAGRAM_STATS_ENABLED or not RAPIDAPI_KEY:
        return []
    endpoint="/tags"
    if not reserve_provider_call("instagram_statistics", endpoint, INSTAGRAM_STATS_MONTHLY_LIMIT):
        return []
    tag=get_today_instagram_tag()
    url="https://instagram-statistics-api.p.rapidapi.com/tags"
    headers={"x-rapidapi-key":RAPIDAPI_KEY,"x-rapidapi-host":"instagram-statistics-api.p.rapidapi.com"}
    try:
        res=session.get(url,headers=headers,params={"tag":tag},timeout=20)
        if res.status_code!=200:
            logging.warning("Instagram Statistics #%s HTTP %s: %s",tag,res.status_code,res.text[:300])
            return []
        data=res.json()
        if isinstance(data,list): items=data
        elif isinstance(data,dict):
            items=data.get("data") or data.get("medias") or data.get("items") or []
            if isinstance(items,dict): items=items.get("medias") or items.get("items") or items.get("posts") or []
        else: items=[]
        if not isinstance(items,list): items=[]
        results=[]
        for item in items[:INSTAGRAM_STATS_QUERY_COUNT]:
            if not isinstance(item,dict): continue
            raw=item.get("caption") or item.get("caption_text") or item.get("text") or item.get("description") or ""
            if isinstance(raw,dict): raw=raw.get("text") or raw.get("caption") or ""
            raw=str(raw).strip()
            if len(raw)<=10: continue
            likes=item.get("likes") if item.get("likes") is not None else item.get("like_count") or item.get("likes_count")
            comments=item.get("comments") if item.get("comments") is not None else item.get("comment_count") or item.get("comments_count")
            views=item.get("views") if item.get("views") is not None else item.get("view_count") or item.get("video_views")
            metrics=[]
            ...
            results.append({"platform":"instagram","query":"","tag":tag,"region":"EU","text":text,"likes":likes,"comments":comments,"views":views})
        logging.info("Instagram Statistics #%s samples=%d; quota=%s",tag,len(results),get_monthly_quota_snapshot())
        return results
    except Exception as e:
        logging.error("Instagram Statistics #%s failed: %s",tag,e)
        return []
...
def main():
    ...
    try:
        ...
        logging.info(
            "Today's Instagram Statistics tag: %s",
            get_today_instagram_tag()
        )
        logging.info(
            "Today's Google groups: %s",
        ...
        logging.info("Fetching Instagram Statistics API...")
        instagram_signals = fetch_instagram_statistics()
